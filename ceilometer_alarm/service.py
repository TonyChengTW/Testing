...
import notifier.sendmail as sendmail
...
cfg.CONF.import_group('send_mail', 'ceilometer.service')
...
class AlarmNotifierService(os_service.Service):
    ...
    def notify_alarm(self, context, data):
        ...
        mail_enable = cfg.CONF.alarm_send_mail.enable

        if mail_enable:
            content = sendmail.SendMailAlarmNotifier.format_content(data)
            sendmail.SendMailAlarmNotifier.alarm_send_mail_method("Ceilometer Alarm!!!", content)
        else:
            LOG.info("Alarm Send Mail is not enabled, do not send mail for alarms!")
